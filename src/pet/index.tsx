import "./style.css";
import Bottom from "../bottom/index";
import Reload from '../../public/reload.png';
import Usuario from '../../public/user.png'
import Jogos from '../../public/games.png'
import Acessar from '../../public/acessarPet.png';

import Doguinho from '../Pets/doguinho.tsx';


export default function Pet() { 
  return (
    <div className="principalConversas">
      <div className="acessosPet">
        <h1>Completo jogos para ganhar recompensas<br></br><p>Nivel: 19</p></h1>
        <div>
          <img src={Acessar} alt="Entrar pet" />
        </div>
      </div>
      <div className="principalPETS">
        <Doguinho />
      </div>
      <div className="conversasFundo">
          <div className="conteudoConversas">
            <ul>
              <li>
                <p>Conexões</p>
              </li>
              <li>
                <img src={Reload} alt="Recarregar pagina" />
              </li>
            </ul>
          </div>
          
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
            <div className="conversas">
              <ul>
                <li className="informacoesConversas">
                  <div>
                    <img src={Usuario} alt="foto do usuario" />
                  </div>
                  <div>
                    <p>Nome de usuario</p>
                    <p>Mensagem</p>
                  </div>
                </li>
                <li>
                  <div>
                    <img src={Jogos} alt="Jogo conectado" />
                  </div>
                </li>
              </ul>
            </div>
          
      </div>      


      <Bottom />

      
    </div>
  );
}
